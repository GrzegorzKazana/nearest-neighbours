...
from constants import ROOM_LABELS
...
def main():
    df = pd.read_csv(os.path.join(data_path, labels_file), usecols=["filename", *ROOM_LABELS])
    ...
    labels_w_imgs = [
        (get_labels_by_filename(df, name), load_image_to_numpy(path, side_length=IMAGE_SIDE_LENGTH)) for path, name in path_w_names
    ]
